...
// Librairie de fonctions pour gerer la session


/*

Gestion de $_SESSION :
    connected : true si connecte, false sinon
    id : id de l'utilisateur connecte


On va stocker l'utilisateur connecte dans une variable globale
...
{
    // Role : initialiser les infos de session
    // Parametres : neant
    // retour : true si on est connecte, false sinon
    session_start();

    // On pourrait ici verifier que l'utilisateur connecte eventuel existe toujours et est autorise a se connecte
...
function connect($id)
{
    // Rôle : déclarer qu'un utilisateur donné est connecté
    // Paramètres : quel utilisateur
    //      $id : id de l'utilisateur a connecter
    // Retour : true
...
function userConnected()
{
    ...
    // Paramètres : néant
    // Il est peut être déjà dans la variable globale $userConnected
    // Variable globale utilisée comme cache.
    // Permet d'éviter de recharger plusieurs fois
    // l'utilisateur connecté depuis la base de données.
    global $userConnected;
...
{
    // Rôle : retourner l'id de l'utilisateur connecté
    // Paramètres : néant
    // Retour : id ou 0
